...
class Opt(object):
    ...
    def cast_float(pred=lambda x: True):
        ...
        predicate (which, by default, allows all values). If it cannot be cast to a float or the
        ...
        return _cast_float
    @staticmethod
    def cast_complex(pred=lambda x: True):
        """
        Casts the value to a complex using the complex() function and then checks it with the
        optional predicate (which, by default, allows all values). If it cannot be cast to a complex
        or the predicate returns False, a ValueError is raised. Additionally, NaN values are never
        allowed.
        """
        def _cast_complex(x):
            ...
            if isnan(x) or not pred(x): raise ValueError
            return x
        return _cast_complex
    @staticmethod
    def cast_number(pred=lambda x: True):
        """
        Casts the value to a number using the int(), float(), or complex() function based on the
        input and then checks it with the optional predicate (which, by default, allows all values).
        If it cannot be cast to a number or the predicate returns False, a ValueError is raised.
        Additionally, NaN values are never allowed.

        The choice of int(), float(), or complex() is done with the following logic:
          * if input is a string:
            * if the input contains 'i' or 'j', complex() is used
            * if the input contains '.', float() is used
            * otherwise int() is used
          * otherwise, the object must extend from Integral, Real, or Complex

        Additionally, a number is simplified as much as possible. If a complex number is generated
        and has an imaginary part of 0, it is converted to a float. If a float is generated by its
        is_integer function returns True, it is converted to an int.
        """
        def _cast_number(x):
            from math import isnan
            ...
            elif not isinstance(x, (Complex,Real,Integral)): raise ValueError
    ...
    @staticmethod
    def cast_tuple_of(cast, min_len=0, max_len=None, seperator=','):
        """
        Casts the value to a tuple of values (given as a comma-seperated list), each that passes
        through the cast function given. Additionally, the minimim and maximum lengths of the
        resulting tuple can be specified (inclusive, default is no restrictions) along with the
        seperator (defaults to a comma).

        If the input is not a string, it is made into a tuple with a single value (if the input is
        not a sequence) or a tuple wit hthe sequence values. cast, min_len, and max_len are still
...
class Help(object):
    ...
    def __msg():
        ...
        print(f18("%s [basic args] [--cmd1 args...] [--cmd2 args...] ..." % program_name))
